...
for x in range(0,100):
    ...
    url = 'https://myanimelist.net/topmanga.php?limit=' + str((x*50))
    response = requests.get(url, timeout=3000)
    content = BeautifulSoup(response.content, "html.parser")
    sleep(1.5)
    for listing in content.find_all('tr', attrs={"class": "ranking-list"}):
        listObject = {
            ...
            "name": listing.find('a', attrs={"class": "hoverinfo_trigger fs14 fw-b"}).text,
        ...
        count+=1
with open('malMData.json', 'w') as outfile:
